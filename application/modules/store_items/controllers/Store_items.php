...
// function of button Show image in the shop 
function view($update_id)
{
    ...
    if($num_rows>0)
    {
        ...
        {
            $gallery_pics[$count]= base_url().'assets/item_galleries_pics/'.$row->picture;
            $count++;
        }
    ...
    //build the breaddcrumbs data array 
...
function _generate_breadcrumbs_array($update_id)
{
    ...
    //figer out what the sub_cat_id is for this item
...
{
    // figer out which the associated sub category has the most items
    ...
    else
    {
        ...
        //which array key paired with the highest value 
...
function get_best_array_key($target_array)
{
    foreach ($target_array as $key => $value)
    {
        ...
        elseif($key>$target_array[$key_with_highest_value])
        {
             $key_with_highest_value=$key; 
        }
    }
...
// for the option of delete item image if some one click Yes- Delete Item our click cancel
function delete($update_id)
{
    ...
    elseif ($submit=="Yes - Delete Item") 
    {
        ...
        $flash_msg="The item was successfull deleted";
...
// for the button of delete item image 
function deleteconf($update_id)
{
    ...
    $data['headline']= "Delete Item Image";
    $data['update_id']=$update_id;
    $data['flash']=$this->session->flashdata('item');  
...
// function for deleting the uploaded image 
function delete_image($update_id)
{
    ...
    $flash_msg="The item image was successfull deleted";
    $value= '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
    $this->session->set_flashdata('item',$value);
...
// function of the button of Upload Item Image 
    ...
    if($submit=="Submit")
    {
        ...
        if ($this->form_validation->run()==TRUE) {
            ...
            else
            {
                ...
                $flash_msg="The item  wase successfully added";
                $value= '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item',$value);
                redirect('store_items/create/'.$update_id);
 ...
 function item_check($str)
{
    ...
    if ($num_rows>0)
    {
           ...
           $this->form_validation->set_message('item_check', 'The title you submitted is not available');
           return FALSE;
    }
